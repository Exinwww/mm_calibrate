...
void ReadPose(std::vector<Eigen::Matrix4d>& lidar_pose, std::string pose_path){
    std::ifstream file;
    file.open(pose_path);
    if (!file.is_open())
    {
        std::cerr << "open file " << pose_path << " failed. Cannot read pose." << std::endl;
        exit(1);
    }
    while (!file.eof())
    {
